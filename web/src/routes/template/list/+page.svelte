...
<div class=" p-4 h-full">
    ...
    <div class="flex flex-col items-center space-y-6">
        ...
        <a href={`${base}/template/view/${set.title}`} class="flex w-3/4 bg-gray-100 hover:bg-gray-200 p-4 cursor-pointer">
            <div class="flex-auto">
                <div class="font-bold text-"> {set.title}</div>
                <div class="flex flex-col space-y-2">
                    <a href={`${base}/publications/template/${set.title}`} class="btn text-white btn-info btn-xs rounded-none">create publication</a>
                    <a href="{base}/template/list" on:click={(e, id)=>deletes(e, set.id)} class="btn btn-xs rounded-none text-white btn-warning">Delete</a>
                </div>
            </div>
